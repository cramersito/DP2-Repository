...
class VerCursosDisponibles extends Simulation {
	...
	val scn = scenario("VerCursosDisponibles")
		.exec(http("Home")
			.get("/")
			.headers(headers_0))
		.pause(5)
	...
	setUp(scn.inject(rampUsers(875) during (100 seconds))).protocols(httpProtocol)
    .assertions(
