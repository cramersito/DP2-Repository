...
class TransportesAcabadosPerfomance extends Simulation {
	...
	val scn = scenario("TransportesAcabados")
		.exec(http("Home")
			.get("/")
			.headers(headers_0)
	...
	setUp(scn.inject(rampUsers(600) during (100 seconds))).protocols(httpProtocol)
    .assertions(
